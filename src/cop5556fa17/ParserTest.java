...
public class ParserTest {
	...
	public void _expression3() throws SyntaxException, LexicalException {
		...
		Declaration_Image dec1 = (Declaration_Image) ast.decsAndStatements
				.get(0);
		Statement_In stmt1 = (Statement_In) ast.decsAndStatements.get(1);
		Statement_In stmt2 = (Statement_In) ast.decsAndStatements.get(2);
		Statement_Out stmt3 = (Statement_Out) ast.decsAndStatements.get(3);
		Declaration_Image dec2 = (Declaration_Image) ast.decsAndStatements.get(4);
		assertNull(dec1.xSize);
		assertNull(dec1.ySize);
		assertEquals("abcd", dec1.name);
		assertNull(dec1.source);
		assertEquals("abcd", stmt1.name);
		Source_StringLiteral s = (Source_StringLiteral) stmt1.source;
		assertEquals("hello", s.fileOrUrl);
		assertEquals("abcd", stmt2.name);
		Source_Ident s1 = (Source_Ident) stmt2.source; 
		assertEquals("pqrs", s1.name);
		assertEquals("abcd", stmt3.name);
		Sink_SCREEN s2 = (Sink_SCREEN)stmt3.sink;
		assertEquals(KW_SCREEN, s2.kind);
		Expression_Ident xsize = (Expression_Ident)dec2.xSize;
		Expression_Ident ysize = (Expression_Ident)dec2.ySize;
		Source_CommandLineParam s3 = (Source_CommandLineParam)dec2.source;
		Expression_Binary paramNum = (Expression_Binary)s3.paramNum;
		Expression_Ident e0 = (Expression_Ident) paramNum.e0;
		Expression_Ident e1 = (Expression_Ident) paramNum.e1;
		assertEquals("hello", dec2.name);
		assertEquals("p", xsize.name);
		assertEquals("q", ysize.name);
		assertEquals("ab",e0.name);
		assertEquals(OP_PLUS,paramNum.op);
		assertEquals("cd",e1.name);
	}
	...
	public void testcase10() throws SyntaxException, LexicalException {
		String input = "++--x?x?R:a:abs[A,R]";//"cart_x[a,r]";//"sin(3)";//"abs(-3)";//"atan(3)";//"cos(3)";//"polar_a[x,y]";//"cart_y[a,r]";//"p image1 = Z;";//"prog @expr k=12;"; 
		show(input);
		Scanner scanner = new Scanner(input).scan();  
		show(scanner);   
		Parser parser = new Parser(scanner);
	...
	public void testcase14() throws SyntaxException, LexicalException {
		String input = "polar_r[x,y]";//"isUrl url filepng=\"abcd\"; \n @expr=12; url awesome=@expr; \n url filepng=abcdefg"; 
		show(input);
		Scanner scanner = new Scanner(input).scan();  
		show(scanner);   
		Parser parser = new Parser(scanner);
	...
	public void testcase16() throws SyntaxException, LexicalException {
		String input = "p[polar_r[x,y], p2[+x,-y]]";//"isFile file filepng=\"abcd\"; \n @expr=12; url filepng=@expr; \n url filepng=abcdefg"; 
		show(input);
		Scanner scanner = new Scanner(input).scan();  
		show(scanner);   
		Parser parser = new Parser(scanner);
